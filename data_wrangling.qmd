...
```{r convert-nutritional-indicators}
df_clean <- 
  df_clean |>
  mutate (
...
```{r remove-additional-redundant-features}
df_clean <-  df_clean |>
  select(
    -c(
      # Domain knowledge
...
```{r create-summary-eda}
gt_plt_summary(df_clean)
```

```{r export-dataset}
# Export wrangled dataset
export(df_clean, here("data", "df_wrangled.RData"))

export(df_clean, here("data", "df.csv"))
```
...
df_clean <- df_clean |>
  select(age_days, sex, child_weight_kg, child_lenhei_cm, everything())
...
df_clean <- 
  ...
  mutate (
    ...
    wealth_index_decile = wealth_index_decile |>
      fct_relevel(
      "1st decile",
      "2nd decile",
      "3rd decile",
...
```{r}
df_clean |>
  select(age_days, sex, child_weight_kg, child_lenhei_cm, area, mother_educational_level, stunting) |>
  tbl_summary(
    label = c(
      age_days ~"Age (days)",
      sex ~ "Sex",
      child_weight_kg ~ "Weight (kg)",
      child_lenhei_cm ~ "Length/Height (cm)",
      area ~ "Area",
      mother_educational_level ~ "Mother's educational level",
      stunting ~ "Stunting"
    )
  )
```
...
#       child_weight_kg ~ "Weight (kg)",
#       child_lenhei_cm ~ "Length/Height (cm)",
#       child_had_diarrhoea_last_two_weeks ~ "Child had diarrhoea last two weeks",
#       child_ill_fever_two_weeks ~ "Child ill with fever last two weeks",
#       child_ill_cough_two_weeks ~ "Child ill with cough last two weeks",
#       area ~ "Area",
#       region ~ "Region",
#       mother_educational_level ~ "Mother's educational level",
#       health_insurance ~ "Health insurance",
#       mother_functional_disability_18_49 ~ "Functional disability of mother",
#       ethnicity ~ "Ethnicity",
#       combined_wealth_score ~ "Combined wealth score",
#       wealth_index_quintile ~ "Wealth index (Quintile)",
#       wealth_index_decile ~ "Wealth index (Decile)",
#       stunting ~ "Stunting"
#     )
#   )
```
